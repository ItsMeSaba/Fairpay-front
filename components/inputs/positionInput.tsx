...
export default function PositionInput(args: Args) {
    const { state, setState, displayLabel, customStyle } = args;
    ...
    const shouldDisplayList = positions.length > 0 && isFocused

    return (
        <div className={`${style.inputContainer}`} style={customStyle}>
            { displayLabel && <label htmlFor="positionInput">პოზიცია</label> }
            
            <input autoComplete="off" id="positionInput" placeholder={!displayLabel ? "პოზიცია" : ""} list="positionInput" value={state} onChange={handleInput} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} />

            <datalist id="positionInput" style={{ display: shouldDisplayList ? "block" : "none" }}>
